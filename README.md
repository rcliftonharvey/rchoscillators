...
Overall, including the sine wave at the base frequency, it's necessray to calculate and add **2.204** sine waves to get a clean band-limited saw wave at 10 Hz.

22 in contrast to 2.204... quite a jump.
...
As a reference: the lowest (useful) MIDI note is A0, equivalent to a frequency of 27.5 Hz. Using the 22.050 Hz nyquist limit for 44.100 Hz sample rate, that would mean generating 801 partial sine waves.

If you didn't know: there is no band-limited sine wave mode. Because a sine wave does not have any harmonic overtones except its own base frequency, it is basically band-limited by design. That's why it's so reliable and convenient to use as a building block for other, more complex wave shapes.
...
I also wish to extend a big *thank you* to **William Rodewald** for enlightening me over a cup of coffee with what I would call *un-branched conditional evaluations*. Knowing that branched IF blocks are a performance downer in per-sample tick code is only half the rent, but actually finding a more performant alternative is the part that matters. 
